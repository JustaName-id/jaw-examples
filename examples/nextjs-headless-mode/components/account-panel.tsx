...
export function AccountPanel() {
  ...
  useEffect(() => {
    try {
      ...
      setStoredAccounts(accounts as any ?? []);
    } catch {
      // No stored accounts
    }
  }, [apiKey, account]);
          ...
          <div className="space-y-2">
            {storedAccounts.map((stored) => (
              <button
                ...
                disabled={isLoading}
                className="flex w-full items-center justify-between rounded-lg border border-gray-700 bg-gray-800 px-4 py-3 text-left transition-colors hover:border-gray-600 hover:bg-gray-750 disabled:opacity-50"
              >
