...
// ---------------------------------------------------------------------------
// Setup — create a Turnkey wallet and upgrade it via EIP-7702
// ---------------------------------------------------------------------------
...
async function setup() {
  ...
  // Create viem LocalAccount from Turnkey
  console.log('\n  Creating viem account...');
  const localAccount = await getViemAccount(turnkey, env.turnkeyOrgId, walletAddress);

  // EIP-7702: the { eip7702: true } flag preserves the EOA address
  console.log('  Creating JAW smart account with EIP-7702...');
  const account = await Account.fromLocalAccount(
    { chainId: CHAIN_ID, apiKey: env.jawApiKey },
    localAccount,
    { eip7702: true },
  );

  console.log(`  Smart Account : ${account.address}`);
  console.log(`  Same address  : ${walletAddress === account.address}`);
...
async function sendTransaction(account: Account) {
  ...
  const recipient = (to.trim() || account.address) as `0x${string}`;
  ...
  try {
    ...
    console.log(`  Fund the EOA: ${account.address}`);
...
async function menu(account: Account) {
  while (true) {
    ...
    switch (choice.trim()) {
      case '1': await signMessage(account); break;
      case '2': await sendTransaction(account); break;
      case '3':
